...
export class SeasonManager extends BaseManager {
  public async list (offset?: number, maxCount?: number) {
    const rawData = <Array<any>> await this.requestPaginatedResource('seasons', offset, maxCount)

    return rawData.map((data: any) => new Season(this.client, data))
  }

  public async getUpcoming (offset?: number, maxCount?: number) {
    const rawData = <Array<any>> await this.requestPaginatedResource('seasons/upcoming', offset, maxCount, { disableCaching: 'true' })
  ...
  public async get (season: SeasonType, year: number = new Date().getFullYear(), offset?: number, maxCount?: number) {
    const rawData = <Array<any>> await this.requestPaginatedResource(`seasons/${year}/${season.toLowerCase()}`, offset, maxCount)

    return rawData.map((data: any) => new Anime(this.client, data))
  }

  public async getNow (offset?: number, maxCount?: number) {
    const rawData = <Array<any>> await this.requestPaginatedResource('seasons/now', offset, maxCount, { disableCaching: 'true' })
